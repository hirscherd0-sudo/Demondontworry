...
const rooms = {};
const socketRoomMap = {};
const TURN_TIMEOUT = 30000; // 30 Sekunden für entspannteres Spielen
...
function nextTurn(roomId) {
    ...
    // Server Timer (Backup, falls Client hängt)
...
io.on('connection', (socket) => {
    socket.on('joinGame', (roomId) => {
        ...
        if (room.players.length < 4) {
            ...
            // WICHTIG: Sende dem Spieler seine Identität UND Farbe für die Kamera
            socket.emit('setIdentity', { color: c, figure: figures[c], isHost: room.host === socket.id });
            
            // Update Lobby für alle
